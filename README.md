...
这是一个用于学习的图书管理系统后端项目，基于 Spring Boot、MyBatis-Plus、MySQL 和 JWT 实现。项目提供管理员和普通用户两类角色，支持图书管理、用户管理、借阅申请、审批、详情查看和管理员归还登记等功能。
...
- 角色权限：
  ...
  - `READER`：浏览图书、提交借阅申请、查看自己的申请和详情。
- 图书管理：分页查询、新增、编辑、删除、库存维护。
- 用户管理：分页查询、新增、编辑、启用/禁用、重置密码。
- 借阅申请：提交申请、管理员审批、审批详情、管理员登记归还后恢复库存。
...
后端源码已经补充方法级注释。更多关于前后端分离、Spring Boot 分层、Maven、常用注解、JWT 过滤器、日志、MyBatis-Plus 和 Vue/Element Plus 的说明，见：

```text
docs/learning-guide.md
```

如果想按“页面 -> 接口 -> Controller -> Service -> Mapper -> 数据库”的路径查看完整流程图，见：
